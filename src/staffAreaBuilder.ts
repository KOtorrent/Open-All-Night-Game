...
/**
 * Reworks the original temporary rear-room silhouettes into three readable zones:
 * enclosed manager office on the far left, stock/utility space in the middle,
 * and the existing restroom/cooler side on the right.
 */
export function buildStaffArea(app: pc.Application, world: BuiltWorld): void {
  ...
  // Move temporary stock fixtures out of the future office footprint.
  const shelfA = app.root.findByName('StockShelfA');
  const shelfB = app.root.findByName('StockShelfB');
  shelfA?.setPosition(-5.05, 1.35, -9.55);
  shelfB?.setPosition(-4.35, 1.35, -10.15);
  for (let i = 0; i < 7; i++) {
    app.root.findByName(`StockBox-${i}`)?.setPosition(-4.85 + (i % 2) * 0.62, 0.45 + (i % 3) * 0.58, -10.85 + (i % 2) * 1.05);
  }

  // Move the temporary desk/monitor to the enclosed office.
  ...
  // Manager office shell, with a real doorway opening in the front wall.
  ...
  // Office props: chair, filing cabinet, bulletin board, schedule sheets and desk lamp.
  ...
  // Stock-room utility light and relocated collision for moved shelving.
  ...
  // Existing shelf colliders cannot be moved, so add accurate blockers at the new positions.
  collider(world, -5.05, -9.55, 0.70, 4.0, 'Relocated stock shelf A');
  collider(world, -4.35, -10.15, 0.70, 2.6, 'Relocated stock shelf B');

  // Make the monitor visually easier to identify from the office doorway.
