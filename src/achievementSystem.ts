...
export class AchievementSystem {
  ...
  private test(def: AchievementDefinition): boolean {
    ...
    switch (def.id) {
      ...
      case 'EMPLOYEE_OF_THE_MONTH': return (this.state.isComplete('night1-clock-out') && !this.hasKnownRuleBreak() && CHORE_IDS.every((id) => this.state.isComplete(id))) || p.runs.some((r) => r.ruleBreaks === 0 && r.choresCompleted >= 5);
      ...
      case 'PERFECT_WEEK': return [1,2,3,4,5].every((n) => p.runs.some((r) => r.night === n && r.ruleBreaks === 0));
